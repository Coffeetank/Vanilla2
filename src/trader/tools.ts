...
export const getSymbolFiltersTool: ToolDefinition = {
  description: 'Get symbol filters including LOT_SIZE (minQty, maxQty, stepSize), PRICE_FILTER, and MIN_NOTIONAL requirements. ALWAYS call this before placing orders to ensure quantities meet exchange requirements. Quantities must be rounded to stepSize increments and be >= minQty. Returns symbol, lotSize {minQty, maxQty, stepSize}, priceFilter, and notional {minNotional}.',
  parameters: z.object({
...
export const openPositionWithProtectionTool: ToolDefinition = {
  description: 'Place a cross margin order (MARKET/LIMIT) with optional auto-borrow and attach OCO TP/SL. AUTO_BORROW_REPAY automatically borrows: USDT for BUY orders (longs), base asset (BTC/ETH/etc) for SELL orders (shorts). Returns order and protection status.',
  parameters: z.object({
    symbol: z.string().describe('Binance trading symbol, e.g., BTCUSDT or BTC/USDT'),
    side: z.enum(['BUY', 'SELL']).describe('Entry direction: BUY (long) or SELL (short). AUTO_BORROW_REPAY borrows base asset for SELL orders.'),
    ...
    sideEffectType: z.enum(['NO_SIDE_EFFECT', 'MARGIN_BUY', 'AUTO_REPAY', 'AUTO_BORROW_REPAY']).optional().describe('Borrow/repay behavior. AUTO_BORROW_REPAY borrows USDT for BUY (longs) or base asset for SELL (shorts)'),
...
export const manualBorrowTool: ToolDefinition = {
  description: 'Manually borrow asset in cross margin to leverage trading. Usually not needed as AUTO_BORROW_REPAY handles borrowing automatically. Only use this for specific advanced scenarios. Amount must be a string (e.g., "100.00" not 100). Always check getMaxBorrowable first.',
  parameters: z.object({
...
export const manualRepayTool: ToolDefinition = {
  description: 'Manually repay borrowed asset in cross margin. Use this to reduce liabilities and improve margin level. CRITICAL: Only repay assets NOT currently supporting open positions to avoid liquidation. Always check getCurrentPositions first to verify the borrowed asset is not in use. Amount must be a string (e.g., "100.00" not 100).',
  parameters: z.object({
    asset: z.string().describe('Asset to repay, e.g., USDT, BTC'),
    amount: z.string().describe('Repay amount as a string per Binance API, precision-sensitive (include borrowed amount + interest)'),
  }),
  execute: async ({ asset, amount }: { asset: string; amount: string }) => {
